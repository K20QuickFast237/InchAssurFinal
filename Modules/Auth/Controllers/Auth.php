...
class Auth extends BaseController
{
    ...
    protected $helpers = ['text', 'Modules\Images\Images', 'sms'];
    ...
    public function create()
    {
        ...
        try {
            ...
            Events::trigger('newRegistration', $user, $codeConnect);
    ...
    public function codeConfirm()
    {
        ...
        $utilisateurID = model("UtilisateursModel")->where('user_id', $userID)->findColumn('id')[0];
    ...
    public function create2()
    {
        $rules = [
            ...
            'password' => ['rules' => 'required', 'errors' => ['required' => 'Valeur Inappropriée.']],
            'categorie' => ['rules' => 'required', 'errors' => ['required' => 'Valeur Inappropriée.']],
            'email'   => [
            ...
            'tel1'    => [
                'rules' => 'required|is_unique[IA_utilisateurs.tel1]|min_length[8]',
                'errors' => ['is_unique' => 'Téléphone déjà existant.', 'required' => 'Valeur Requise.', 'min_length' => 'Valeur Inappropriée.']
            ],
        ];
        ...
        if (isset($input['email']) && isset($input['tel1'])) {
            $modelUser  = new UtilisateurModel(); // are $modelUser = model('UtilisateurModel');
            ...
            $modelUser->set('unique_id', $userID)->where('id_utilisateur', $userID)->update();

            $UserEmail = $UsersInfos['email'];
            // $user = $modelUser->asArray()->where('email', $UserEmail)->first();
            ...
            $modelConnexion = model("ConnexionsModel");
            $modelConnexion->save($UserConnexion);

            // Utilisateur profil sauvegarde
            $this->createProfil($userID, $input['categorie'], 1);
            $this->db->transCommit();
            $response = [
                'statut' => 'ok',
                'message' => 'Un mail d\'activation à été envoyé dans votre boite mail',
     ...
     * signIn()
     * 
     * Connexion d'un utilisateur à la plateforme
     *
     * @return \CodeIgniter\HTTP\ResponseInterface
     * 
     */
    public function signIn2()
    {
        ...
        if ((isset($input['email']) || isset($input['tel1'])) && isset($input['password'])) {
            $input['email'] = strtolower($input['email']);
            $connectModel = new ConnexionModel();
            $User  = new UtilisateurModel();
            if (isset($input['email'])) {
                $userconnect = $connectModel->asArray()->where('email', $input['email'])->first();
            }

            if (is_null($userconnect)) { // this case is for a not account or bad email and password
                $response = [
                    'statut'        => 'no',
            ...
            if ($userconnect['codeconnect'] != "000000") {
                $response = [
                    'statut' => 'no',
                    'message' => 'Consultez votre boite Mail et Activez votre compte.',
                ];
                /** @todo penser à renvoyer l'email au besoin */
                return $this->getResponse($response, ResponseInterface::HTTP_NOT_ACCEPTABLE);
            } else {
                $password = $this->hashpass($input['password']);
                if ((
                    (isset($input['email']) && ($input['email'] == $userconnect['email']))
                    || (isset($input['tel1']) && ($input['tel1'] == $userconnect['tel1']))
                ) && ($password == $userconnect['password'])) {

                    $User->setConnectedState($input['email']);
                    $profils = $this->getUserProfils($input['email']);
                    $tokenprofil = $this->getTokenUserProfil($input['email']);

                    $token = makePersonalisedToken(['email' => $input['email'], 'profil' => $tokenprofil], self::VALIDITE);
                    // $data['token'] = $token;
                    $data['profils'] = $profils;
                    $data['profil'] = $tokenprofil;
                    $response = [
                        'statut'        => 'ok',
            ...
            $response = [
                'statut' => 'no',
                'message' => 'Identifiant où mot de passe introuvable',
            ];
            return $this->getResponse($response, ResponseInterface::HTTP_EXPECTATION_FAILED);
        }
    }
    ...
    public function codeConfirm2()
    {
        ...
        $portefeuilleModel = new PortefeuilleModel();
        $userConnexion = $connexion->where('codeconnect', $input['code'])->first();
        ...
        if ($code == $input['code']) {
            ...
            $infoPortefeuille = [
                'solde' => 0,
                'devise' => 'XAF',
                'utilisateur_id' => $userConnexion->utilisateur_id,
            ];
            $portefeuilleModel->insert($infoPortefeuille);
        ...
        return $this->getResponse($response, ResponseInterface::HTTP_LOCKED); //erreur inconnue
    ...
    public function resetCodeConfirm()
    {
        ...
        $modelUser  = new UtilisateurModel();
        $user       = $modelUser->asArray()->where('email', $this->request->userEmail)->first();

        $codeconnect = $this->getCodeconnect(6);
        $connexion->where('email', $user['email'])
            ->set('codeconnect', $codeconnect)
            ->update();

        $nomComplet = $user['nom'] . " " . $user['prenom'];
        $token = $this->sendmail($user['email'], $nomComplet, $codeconnect);
    ...
    public function resetPassword()
    {
        ...
        // recupere le token en entete
        /*try{
            $token = getJWTFromRequest($this->request->getServer('HTTP_AUTHORIZATION'));
        }catch (\Exception $e){
            $response = ['statut' => 'no',
                         'message'=> 'aucun token',
                        ];
        ...
        // on recupere l'email contenu dans le teken d'entete
        /*try{
        ...
        $userConnexion = $connexion->where('email', $this->request->userEmail)->first();
        $connexion->update(
            $userConnexion->id_connexion,
            // ['password' => $this->hashpass($input['password'])]
            ['password' => $input['password']] // unfiltre dans le controleur fait le hash
        );
    ...
    {
        $user = new UtilisateurModel();
        $user->setDisconnectedState($this->request->userEmail);
        return $this->getResponse(['statut' => 'ok', 'message' => 'Déconnection Réussie!!',]);
    }
     ...
     * @return jsonWebToken|false
     */
    public function sendResetPwdEmail($recipients)
    {
        $email = Services::email();
        $front_baseURL = Services::getforntBaseUrl();
        ...
        // generation du token du lien
        helper('jwt');
        $token = makePersonalisedToken(['email' => $recipients], 7776000);

        $email->setFrom('nanguedevops@gmail.com', 'IncH Assurance');
        $email->setTo($recipients);
        $email->setCC(['nsangouassanzidan@gmail.com', 'tonbongkevin@gmail.com']);
        $email->setSubject('Password Reset');
        $email->setMessage("<h4>Suivez ce lien pour reinitialiser votre mot de passe.</h4>
                            <a href='" . $front_baseURL . "nouveau-mot-passe/" . $token . "'>Reinitialiser</a> mon mot de passe.");
        $tentative = 0;
        while ($tentative < 3) {
            try {
                $email->send();
